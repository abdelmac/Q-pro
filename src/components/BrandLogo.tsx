...
export default function BrandLogo({ className = 'w-44', variant = 'horizontal', label = 'Specialty Match' }: BrandLogoProps) {
  const artwork = <image href={logoUrl} width="1254" height="1254" />;
  return (
    <svg
      ...
      aria-label={label}
      focusable="false"
      viewBox={variant === 'horizontal' ? '0 0 354 64' : variant === 'mark' ? '402 284 448 444' : '100 282 1054 688'}
      className={`block h-auto max-w-full shrink-0 mix-blend-multiply ${className}`}
      xmlns="http://www.w3.org/2000/svg"
    >
      {variant === 'horizontal' ? <>
        <svg x="0" y="0" width="64" height="64" viewBox="402 284 448 444" overflow="hidden" aria-hidden="true">{artwork}</svg>
