...
export default function Intro({ onStart, totalQuestions, participantRole, onBack, onChangeRole, onOpenExplorer, onOpenMethodology, onOpenDashboard }: IntroProps) {
  ...
  const RoleIcon = isSpecialist ? Stethoscope : isCurious ? Compass : GraduationCap;
  const roleLabel = isSpecialist ? t.specialistMode : isCurious ? t.curiousMode : t.studentMode;
  const introBadge = isSpecialist ? t.specialistBadge : isCurious ? t.curiousIntroBadge : t.forMedicalStudents;
