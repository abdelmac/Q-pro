...
8. Après le profil de traits, l’étudiant et la personne qui explore la médecine peuvent répondre à « Que pensez-vous de la médecine ? ». Cette réponse est facultative : vide ou composée seulement d'espaces, elle est enregistrée comme `NULL`. Le texte d'information précise les données enregistrées et l'action d'enregistrement vaut consentement ; le bouton permettant de passer toute la contribution reste disponible.
...
| Regard sur la médecine | `medicine_view` | Verbatim facultatif. Lorsqu’il est fourni, il est normalisé uniquement aux extrémités, conservé fidèlement et doit contenir 3 à 2 000 caractères ; une saisie vide ou composée seulement d’espaces devient `NULL`. |
...
| Provenance du catalogue | `specialty_config_version_id`, `specialty_config_revision` | Instantané publié exact utilisé par les soumissions v2 à v5 ; obligatoire pour les schémas 2, 3 et 4. |
...
Point d'entrée antérieur conservé pour compatibilité. Il conserve les 81 items et le catalogue de valeurs, puis ajoute deux données obligatoires au moment d'enregistrer volontairement une réponse :
...
Point d'entrée courant des étudiants et des personnes qui explorent la médecine. Il conserve le payload quantitatif complet et le rôle explicite, mais rend `medicine_view` facultatif. Une valeur `NULL`, vide ou composée seulement d'espaces est normalisée en `NULL`. Lorsqu'un texte est fourni, il doit compter entre 3 et 2 000 caractères après suppression des seuls espaces périphériques, avec une limite de 8 000 octets.

La fonction impose `client-scoring-v2`, `research-consent-2026-09-16`, `medicine-view-optional-v2` et la provenance exacte d'un catalogue publié, puis écrit `submission_schema_version = 4`. La version du protocole est toujours enregistrée, même si le texte est absent, afin d'indiquer que la question facultative a bien été présentée. L'année d'études reste facultative de 1 à 6 pour `student` et interdite pour `curious`. L'idempotence compare `medicine_view` avec une égalité compatible avec `NULL` : un rejeu strictement identique renvoie le même UUID, tandis qu'un payload différent est refusé.
...
Les contraintes de table distinguent cinq générations sans réinterpréter les anciennes lignes :
...
- le schéma 3 étend les réponses participantes avec un rôle explicite, un verbatim obligatoire sur la médecine et la version de cette question ;
- le schéma 4 conserve ce rôle et cette provenance, mais rend le verbatim facultatif.

Pour les schémas 2, 3 et 4, la base contrôle notamment :

- pour les participants des schémas 3 et 4 et les spécialistes ayant choisi le profil quantitatif : exactement 81 identifiants, des notes entières de 1 à 10 et une à quatre valeurs canoniques distinctes ;
...
- pour le schéma 3 historique, un avis sur la médecine obligatoire de 3 à 2 000 caractères et la version `medicine-view-v1` ;
- pour le schéma 4, la version `medicine-view-optional-v2` et un avis soit `NULL`, soit normalisé de 3 à 2 000 caractères et au plus 8 000 octets ;
...
Les contrôles existent à la fois dans les RPC et dans des contraintes `CHECK`, afin qu'une erreur interne ne puisse pas créer silencieusement une ligne de schéma 2, 3 ou 4 incohérente. Les lignes historiques restent consultables et exportables ; les schémas participants 2 et 3 restent quantitativement analysables lorsqu'ils respectent leur propre provenance, sans être confondus avec le filtre du protocole courant.
...
- les appels v2, v3, v4 et v5 refusent une version non publiée ;
- l’UUID et la révision enregistrés sont cohérents ;
- les fonctions v1 et v2 restent compatibles avec les données historiques ;
- la soumission participante v4 historique continue d'exiger son verbatim et reste compatible ;
- la soumission participante v5 distingue `student` et `curious`, accepte un verbatim absent comme `NULL`, valide tout texte fourni, interdit l'année d'études aux explorateurs et écrit le schéma 4 avec les versions scientifiques exactes ;
- un rejeu v5 avec un verbatim `NULL` reste idempotent, tandis qu'un même UUID accompagné d'un payload différent est refusé ;
