...
/*
 * Schema 4 keeps the versioned participant-reflection protocol, but makes
 * the answer itself optional. A blank or whitespace-only answer is stored as
 * NULL so absence is explicit and cannot be confused with qualitative data.
 * Schema 0-3 rows and the v4 RPC remain valid for historical reproducibility.
 */
COMMENT ON COLUMN public.student_responses.medicine_view IS
  'Optional participant view of medicine. Schema-4 submissions store NULL for an omitted or whitespace-only answer; supplied answers are trimmed and contain 3-2000 characters.';
COMMENT ON COLUMN public.student_responses.participant_reflection_version IS
  'Version of the qualitative medicine-perception prompt and collection protocol, including whether the answer was optional.';

ALTER TABLE public.student_responses
  DROP CONSTRAINT IF EXISTS student_responses_payload_v3_check;
...
ALTER TABLE public.student_responses
  ...
  CHECK (
    ...
    OR (
      ...
      AND specialty_catalog_version = 'medical-specialties-v1'
      AND scoring_version = 'client-scoring-v2'
      AND consent_version = 'research-consent-2026-09-16'
      AND specialty_config_version_id IS NOT NULL
      AND specialty_config_revision IS NOT NULL
      AND private.valid_ratings_q81_v1(ratings)
      AND private.valid_selected_values_v1(selected_values)
...
CREATE OR REPLACE FUNCTION private.submit_student_response_v5(
...
BEGIN
  ...
  IF p_submission_id IS NULL
     ...
     OR p_consent_version IS DISTINCT FROM 'research-consent-2026-09-16'
...
CREATE OR REPLACE FUNCTION public.submit_student_response_v5(
  p_submission_id uuid,
  p_participant_role text,
  p_medicine_view text,
  p_study_year integer,
...
SET search_path = ''
AS $$
  SELECT private.submit_student_response_v5(
    p_submission_id,
    p_participant_role,
    p_medicine_view,
    p_study_year,
...
REVOKE ALL ON FUNCTION private.submit_student_response_v5(
  uuid, text, text, integer, text, jsonb, jsonb, jsonb, text, text, text,
  text, text, text, text, uuid
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.submit_student_response_v5(
  uuid, text, text, integer, text, jsonb, jsonb, jsonb, text, text, text,
  text, text, text, text, uuid
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.submit_student_response_v5(
  uuid, text, text, integer, text, jsonb, jsonb, jsonb, text, text, text,
  text, text, text, text, uuid
) TO anon, authenticated;

COMMENT ON FUNCTION public.submit_student_response_v5(
  uuid, text, text, integer, text, jsonb, jsonb, jsonb, text, text, text,
  text, text, text, text, uuid
) IS
  'Validated idempotent schema-4 submission for medical students and medicine explorers; the versioned medicine-view answer is optional and blank input is stored as NULL.';

COMMENT ON FUNCTION private.submit_student_response_v5(
  uuid, text, text, integer, text, jsonb, jsonb, jsonb, text, text, text,
  text, text, text, text, uuid
) IS
  'Private validated writer for schema-4 student and medicine-explorer research submissions with an optional qualitative medicine view.';
