...
for (const [density, size, foregroundSize] of [
  ...
  for (const name of ['ic_launcher', 'ic_launcher_round', 'ic_launcher_foreground']) {
    ...
    verifyPng(await load(path), name === 'ic_launcher_foreground' ? foregroundSize : size, path);
  }
}
...
if (buildDirectory) {
  ...
  for (const filename of ['favicon.ico', ...publicPngs.map(([name]) => name)]) {
...
console.log(`App icons: ${checkedPngs} valid PNG exports, multi-resolution ICO, untouched supplied artwork, base-aware HTML/manifest, Android adaptive assets and opaque iOS icon passed${buildDirectory ? `; ${buildDirectory} URLs and files passed` : ''}.`);
