...
const migrationPath = path.join(
  ...
  'supabase',
  'migrations',
  '20260905073000_multilingual_specialty_narratives.sql',
);
...
const secondPersonPatterns = {
  en: /(?<![\p{L}\p{M}])(?:you|your|yours)(?![\p{L}\p{M}])/iu,
  ro: /(?<![\p{L}\p{M}])(?:tu|te|tine|ești|esti|îți|iti|tău|tale)(?![\p{L}\p{M}])/iu,
  fr: /(?<![\p{L}\p{M}])(?:tu|vous|votre|vos|tes|ton|ta)(?![\p{L}\p{M}])/iu,
};
...
async function loadAndValidateNarratives() {
  ...
  for (const name of expectedNames) {
    ...
    for (const language of languages) {
      assert(
        !secondPersonPatterns[language].test(narrative.fitProfile[language]),
        `${name}.fitProfile.${language} must use neutral third-person language`,
      );
    }
...
/*
 * Publishes the reviewed multilingual specialty narratives supplied for the
 * results experience. Matching profiles and their weights are copied without
 * modification. The previous active snapshot remains archived and auditable.
...
BEGIN
  ...
  INSERT INTO private.specialty_catalog_versions (
    ...
    'medical-specialties-multilingual-pending',
    'draft',
    1,
    active_version.id,
    'Reviewed Romanian, French, and English specialty narratives supplied for the complete Top-10 results view',
    NULL
  )
